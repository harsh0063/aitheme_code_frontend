...
import { useLoginUserMutation } from '../service/apislice';
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        localStorage.setItem("aithemetoken", response?.access);
  ...
  return (
    <section className="min-h-screen flex items-center justify-center  px-4 bg-blue-50">
      ...
      <div className="bg-white rounded-2xl shadow-[0px_9px_50px_1px_#3DB0F733] w-full max-w-md sm:min-w-[400px] md:min-w-[500px] lg:min-w-[600px] max-sm:p-4 p-8 mx-auto">
        ...
        <p className="text-gray-400 text-center mb-6 text-[15px] max-sm:text-[12px]">Log in to your account</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between text-sm mb-6">
            ...
            <Link to="/forgot_password" title="Forgot Password" className="text-black hover:underline">
              Forgot Password?
            </Link>
          </div>
        ...
        <p className="text-sm text-center mt-6">
          ...
          <Link to="/register" title="register" className="text-primary hover:underline">
            Signup
          </Link>
        </p>
      </div>
